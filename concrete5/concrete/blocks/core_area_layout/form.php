...
<ul id="ccm-layouts-toolbar" class="ccm-inline-toolbar ccm-ui">
    ...
    ?>
				    <option value="<?php echo $pr->getIdentifier()?>" <?php if (isset($selectedPreset) && is_object($selectedPreset) && $selectedPreset->getIdentifier() == $pr->getIdentifier()) {
    ?>selected<?php 
}
    ?>><?php echo $pr->getName()?></option>
				<?php 
}
    ?>
			</optgroup>
    ...
    ?>  data-input="number" data-minimum="<?php echo $minColumns?>" data-maximum="<?php echo $themeGridMaxColumns?>" <?php 
} ?> value="<?php echo $columnsNum?>" />
    ...
    ?> data-input="number" data-minimum="<?php echo $minColumns?>" data-maximum="<?php echo $maxColumns?>" <?php 
} ?> value="<?php echo $columnsNum?>" />
		<?php if ($controller->getTask() == 'edit') {
    // we need this to actually go through the form in edit mode, for layout presets to be saveable in edit mode. ?>
    ...
    ?><?php echo t('Add Layout')?><?php 
} else {
    ?><?php echo t('Update Layout')?><?php 
} ?></button>
	</li>
</ul>
...
$(function() {
    ...
    $class = $classes[$i];
    ?>
				'<?php echo $class?>' <?php if (($i + 1) < count($classes)) {
    ?>, <?php 
}
    ?>
