...
<?php foreach ($accessTypes as $accessType => $title) {
    ...
    ?>
	<a style="float: right" href="<?php echo REL_DIR_FILES_TOOLS_REQUIRED?>/permissions/access_entity?accessType=<?php echo $accessType?>&pkCategoryHandle=<?php echo $pkCategoryHandle?>" dialog-width="510" dialog-height="500" dialog-title="<?php echo t('Add Access Entity')?>" class="dialog-launch btn btn-xs btn-default"><?php echo t('Add')?></a>
...
<tr>
	...
	<a href="<?php echo REL_DIR_FILES_TOOLS_REQUIRED?>/permissions/access_entity?peID=<?php echo $pae->getAccessEntityID()?>&pdID=<?php echo $pdID?>&accessType=<?php echo $accessType?>" dialog-width="510" dialog-height="500" dialog-title="<?php echo t('Add Access Entity')?>" class="<?php if (!is_object($pa->getPermissionDurationObject())) {
    ?>icon-link<?php
}
